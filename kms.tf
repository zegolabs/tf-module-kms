...
data "aws_iam_policy_document" "secret_kms" {
  ...
  dynamic "statement" {
    ...
    content {
      ...
      condition {
        test     = "StringEquals"
        variable = var.test_condition_var_one
        values   = [allowed_reader.value.namespace]
      }

      condition {
        test     = "StringEquals"
        variable = var.test_condition_var_two
        values   = [allowed_reader.value.service]
      }

      condition {
        test     = "StringEquals"
        variable = var.test_condition_var_three
  ...
  dynamic "statement" {
    for_each = !contains(var.protected_owners, var.owner) && length(local.user_roles) != 0 ? [1] : []
