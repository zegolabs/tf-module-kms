locals {
  ...
  account_tfrole = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/tf_b2b_secrets_config"
  ...
  # Secrets owned by those in this list will not be accessible by the user_roles above
  protected_owners = [
    "systems-engineering",
    "security"
  ]
